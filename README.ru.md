...
- ⏰ **Напоминание срабатывает ровно один раз, а доставку проверяет Ива**: строка в `data/` переходит «pending → fired» одним атомарным переходом, и в срок происходят два независимых дела: код сам шлёт ваш текст в чат владельца (без модели и её токенов), а агент просыпается с этим фактом, смотрит список и, если текст не дошёл, говорит сам и объясняет причину. Факт доставки (`fired_at`, `delivered`, `error`) лежит в строке, его показывает `iva doctor`, а строки старой схемы переживают обновление.
